...
description: Apply the Redfearn Group design system and voice to any deliverable for Brady Redfearn / Redfearn Group, personal portfolio, redfearn.group, case studies, blog posts, reports, dashboards, slides, diagrams/data visualizations, GitHub project repos and the real working applications published from them (garage-log, home-log, and any future app), or any other designed or written output. Use whenever Brady asks for a new portfolio piece, case study, blog post, slide, web artifact, landing page, report, diagram, or GitHub project, and whenever creating or restyling an application repo, even if he doesn't name the system explicitly, and whenever writing any Brady-facing copy that should avoid em-dashes and AI-sounding phrasing. This is the canonical, standalone design system and voice guide for Redfearn Group, fully independent of any employer's branding. It defines the three layer scopes (Identity, Content, Product), typography, WCAG-verified color pairings for both light and dark surfaces, status/callout components, signature elements, voice/copy rules, diagram conventions, and known implementation pitfalls. Color values themselves live in brand.css in the redfearn-group/redfearn-brand repo, which this skill points at rather than restating. Supersedes any earlier "barn-swallow-design" skill, use this one instead.
---
...
These are portfolio evidence that Brady ships working software, so they are held to the same brand standard as the site, not treated as throwaway utilities. Requirements:
- Vendor `brand.css`, add the drift-check workflow, set `class="app-layer"` on `<html>`.
- `rg-mark.svg` as the favicon and in the header next to the app name. Never ship a framework default favicon.
